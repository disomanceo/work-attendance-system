...
export async function GET(
  ...
  if (error) {
    console.error("Load announcement Word file metadata error:", error);
    return NextResponse.json(
      { ok: false, message: "ไม่สามารถตรวจสอบไฟล์ประกาศได้" },
      { status: 500 },
    );
  }
  ...
  if (!announcement) {
    return NextResponse.json(
      { ok: false, message: "ไม่พบไฟล์ Word ของประกาศ" },
      { status: 404 },
    );
  }
  ...
  try {
    ...
    const fileName = safeWordFileName(announcement.docx_file_name);
    ...
    console.error("Download announcement Word file error:", downloadError);
    return NextResponse.json(
      { ok: false, message: "ไม่สามารถดาวน์โหลดไฟล์ Word ของประกาศได้" },
      { status: 502 },
    );
  }
}
