...
export async function DELETE(request: Request) {
  try {
    ...
    const { data: existing, error: loadError } = await admin
      .from("leave_requests")
      .select("id, status, working_document_id, drive_request_folder_id, evidence_file_id")
      .eq("user_id", user.id);

    if (loadError) throw new Error("โหลดประวัติการลาสำหรับลบไม่สำเร็จ");

    const rows = existing ?? [];
    if (rows.length === 0) {
      return NextResponse.json({ ok: true, deletedCount: 0, message: "ไม่มีประวัติการลาสำหรับลบ" });
    }
    ...
    if (deleteError) throw new Error("ลบประวัติการลาไม่สำเร็จ");

    return NextResponse.json({
      ok: true,
      deletedCount: rows.length,
      message: `ลบประวัติการลา ${rows.length} รายการเรียบร้อยแล้ว`,
    });
  } catch (error) {
    return NextResponse.json(
      { ok: false, message: error instanceof Error ? error.message : "เกิดข้อผิดพลาด" },
      { status: 500 }
    );
  }
}