...
export default function LeavePage() {
  ...
  async function deleteHistory() {
    if (requests.length === 0 || deletingHistory) return;

    const confirmed = window.confirm(
      "ลบประวัติการลาทั้งหมดของบัญชีนี้หรือไม่? ใช้เฉพาะช่วงทดสอบ"
    );
    if (!confirmed) return;
    ...
    try {
      ...
      if (!response.ok || !result.ok) {
        throw new Error(result.message || "ลบประวัติการลาไม่สำเร็จ");
      }

      setMessage(result.message || "ลบประวัติการลาเรียบร้อยแล้ว");
      await loadData();
    } catch (error) {
      setErrorMessage(
        error instanceof Error ? error.message : "ลบประวัติการลาไม่สำเร็จ"
      );
    } finally {
          ...
          <div className={styles.cardHeading}>
            ...
            <div className={styles.historyActions}>
              ...
              <button
                ...
                disabled={loading || deletingHistory || requests.length === 0}
                onClick={() => void deleteHistory()}
              >
                {deletingHistory ? "กำลังลบ..." : "ลบประวัติ"}
              </button>
            </div>
          </div>
            ...
            <div className={styles.tableWrap}>
              <table className={styles.historyTable}>
                ...
                <tbody>
                  {requests.map((item) => (
                    <tr key={item.id}>
                      <td><span className={styles.leaveType}>{leaveLabel(item.leave_type)}</span></td>
                      <td>
                        {item.leave_number ||
                          (item.sequence_number ? `${leaveLabel(item.leave_type)}ครั้งที่ ${item.sequence_number}` : "-")}
                        <small>{submissionLabel(item.submission_kind)}</small>
                      </td>
                      <td>{item.start_date} ถึง {item.end_date}</td>
                      <td>{item.total_work_days}</td>
                      <td className={styles.reasonCell} title={item.reason}>{item.reason}</td>
                      <td>
                      ...
                      <td>
                        <div className={styles.tableActions}>
                          {item.status === "pending" && item.working_document_url && (
                            <a href={item.working_document_url} target="_blank" rel="noopener noreferrer">
                              ใบรอพิจารณา
                            </a>
                          )}
                          {item.evidence_file_id && (
                            <button type="button" onClick={() => void openAttachment(item.id)}>
                              หลักฐาน
                            </button>
                          )}
                          {item.pdf_file_url && (
                            <a href={item.pdf_file_url} target="_blank" rel="noopener noreferrer">
                              PDF
                            </a>
                          )}
                        </div>
