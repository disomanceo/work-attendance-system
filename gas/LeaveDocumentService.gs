...
function leaveCreatePending_(payload) {
  ...
  const pendingRoot = getOrCreateFolder_(
    yearFolder,
    "เอกสารชั่วคราวรอพิจารณา"
  );

  const leaveNumber = nextLeaveNumber_(
    payload.roleKey,
    fiscalYear
  );
  ...
  try {
    ...
    const applicantSignatureInserted =
    ...
    const replacementReport =
      replaceFieldsInDocument_(document, [
        ...
        field_(
          ["POSITION", "ตำแหน่ง", "ตำแหน่งผู้ลา"],
          payload.position || ""
        ),
        field_(
          ["LEAVE_TYPE", "ประเภทการลา", "ประเภทลา"],
          leaveTypeLabel_(payload.leaveType)
        ),
        field_(
          ["START_DATE", "วันที่เริ่มลา", "ตั้งแต่วันที่"],
          formatThaiDate_(payload.startDate)
        ),
        field_(
          ["END_DATE", "วันที่สิ้นสุด", "ถึงวันที่"],
          formatThaiDate_(payload.endDate)
        ),
        field_(
          ["TOTAL_DAYS", "จำนวนวันลา", "รวมวันลา"],
          String(payload.totalDays || "")
        ),
        field_(
          ["REASON", "เหตุผล", "เหตุผลการลา"],
          payload.reason
        ),
        field_(
          [
        ...
        field_(
          ["SUBMITTED_DATE", "วันที่ยื่น", "วันที่ยื่นใบลา"],
          formatThaiDateTime_(
    ...
    if (!replacementReport.FULL_NAME) {
      requiredMissing.push("ชื่อผู้ลา");
    }

    if (!replacementReport.LEAVE_TYPE) {
      requiredMissing.push("ประเภทการลา");
    }

    if (!replacementReport.START_DATE) {
      requiredMissing.push("วันที่เริ่มลา");
    }

    if (!replacementReport.END_DATE) {
      requiredMissing.push("วันที่สิ้นสุด");
    }

    if (!replacementReport.REASON) {
      requiredMissing.push("เหตุผล");
    }
    ...
    if (requiredMissing.length > 0) {
      throw new Error(
        "แม่แบบไม่พบ Placeholder สำคัญ: " +
          ...
          " กรุณาใช้รูปแบบเช่น {{ชื่อผู้ลา}} โดยต้องพิมพ์ Placeholder เป็นข้อความต่อเนื่อง"
      );
    }
    ...
    if (payload.evidenceBase64) {
      ...
      evidenceFile = requestFolder.createFile(
        evidenceBlob
      );
    ...
    return {
      ...
      evidenceFileId: evidenceFile
        ? evidenceFile.getId()
        : "",
      evidenceFileUrl: evidenceFile
        ? evidenceFile.getUrl()
        : "",
      applicantSignatureInserted:
        applicantSignatureInserted,
      replacementReport: replacementReport
    };
  } catch (error) {
    safeTrashFile_(
      workingFile && workingFile.getId()
    );
    safeTrashFile_(
      evidenceFile && evidenceFile.getId()
    );
    safeTrashFolder_(
      requestFolder && requestFolder.getId()
    );
...
function leaveFinalize_(payload) {
  ...
  const workingFile = DriveApp.getFileById(
    payload.workingDocumentId
  );
  const document = DocumentApp.openById(
    payload.workingDocumentId
  );
  ...
  const decisionText =
    String(payload.decision) === "approved"
      ? "อนุมัติ"
      : "ไม่อนุมัติ";
  ...
  replaceFieldsInDocument_(document, [
    ...
    field_(
      ["REVIEWED_DATE", "วันที่พิจารณา", "วันที่อนุมัติ"],
      formatThaiDateTime_(new Date())
    )
  ]);
  ...
  const rootFolder = DriveApp.getFolderById(
    LEAVE_ROOT_FOLDER_ID
  );
  const yearFolder = getOrCreateFolder_(
    rootFolder,
    "ปีงบประมาณ " + String(payload.fiscalYear)
  );
  const pdfFolder = getOrCreateFolder_(
    yearFolder,
    "ใบลา PDF"
  );
  ...
  if (payload.evidenceFileId) {
    try {
      const evidenceSource = DriveApp.getFileById(
        String(payload.evidenceFileId)
      );
      ...
      const evidenceName =
        ...
        getFileExtension_(evidenceSource.getName());

      finalEvidenceFile = evidenceSource.makeCopy(
        evidenceName,
        pdfFolder
      );
    } catch (error) {
      console.warn(
        "ไม่สามารถคัดลอกไฟล์หลักฐานไปยังโฟลเดอร์ถาวรได้"
      );
    }
  }
  ...
  return {
    ...
    finalEvidenceFileId: finalEvidenceFile
      ? finalEvidenceFile.getId()
      : "",
    finalEvidenceFileUrl: finalEvidenceFile
      ? finalEvidenceFile.getUrl()
      : "",
    finalEvidenceFileName: finalEvidenceFile
      ? finalEvidenceFile.getName()
      : ""
  };
}
...
function verifySecret_(receivedSecret) {
  ...
  if (
    !receivedSecret ||
    String(receivedSecret) !== expectedSecret
  ) {
...
function replaceFieldsInDocument_(
  document,
  fields
) {
  const report = {};
  const containers =
    documentContainers_(document);
  ...
  fields.forEach(function(field) {
    ...
    containers.forEach(
      function(container) {
        field.aliases.forEach(
          function(alias) {
            count += replaceAllText_(
              container,
              alias,
              field.value
            );
...
function insertImageInDocument_(
  document,
  placeholders,
  dataUrl,
  maxWidth
...
function dataUrlToBlob_(
  ...
  if (
    ...
    throw new Error(
      "รองรับไฟล์เฉพาะ PDF, JPG และ PNG"
    );
  }
